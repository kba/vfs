 ...
 * @alias zip
 */
class fsvfsZip extends fsvfs {

    static get scheme() { return 'zip' }

    constructor(options) {
        if (!options.location) throw new Error("Must set 'location'")
        if (!(options.location instanceof fsvfs.Node)) throw new Error("'location' must be a vfs.node")
        super(options)
    }
    ...
    stat(path, cb) {
        if (!(Path.isAbsolute(path))) return cb(fsvfs.PathNotAbsoluteError(path))
        path = Path.normalize(path.substr(1))
        if (!(path in this.zipRoot.files)) path += '/'
        if (!(path in this.zipRoot.files))
            return cb(fsvfs.NoSuchFileError(path))
     ...
     */
    readdir(dir, cb) {
        if (!(Path.isAbsolute(dir))) return cb(fsvfs.PathNotAbsoluteError(dir))
